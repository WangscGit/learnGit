    ...
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
	                        ...
	                        var formType = unescape(r[2]);
	                        var inputValue = selectedShape.properties[key];
	                        var tittleValue = property.title();
                            if(property.id() == "process_id"){
                                inputValue = formType;
                            }
                            if(i==1){
                            	if(properties[0].id() == "process_id"){
                            	var regName = new RegExp("(^|&)" + "presName" + "=([^&]*)(&|$)");
	                            var rName = window.location.search.substr(1).match(regName);
	                            var presName = unescape(rName[2]);
	                			var base64 = new Base64();
	                			presName = base64.decode(presName);
	                			presName = presName.replace(/\"/g, "");
	                            inputValue = presName;  
	                            tittleValue = "processName";
                            	}
                            }
                            var currentProperty = {
                                'key': key,
            	...
            	$scope.safeApply(function () {
            		...
            		if (shapes && shapes.length == 1)
            		{
                    	...
                    	if (!containedStencil) return;
                   		var itemId=containedStencil._jsonStencil.id.split("#")[1];
                   		var title=containedStencil._jsonStencil.title;
            			var canvasCandidate = $scope.editor.getCanvas();
						var overrideid="";
						var name="";
						var no=1;
						if(canvasCandidate.children.length>0){
							var myshapes=canvasCandidate.children;
							var reg = /^\d+(\.\d+)?$/;
							for(var i=0;i<myshapes.length;i++){
								if(myshapes[i].toJSON().stencil.id!=itemId){
									continue;								
								}
								var id=myshapes[i].toJSON().properties.overrideid;
								if(id){
									id=id.split("_")[1];
									
									if(reg.test(id)&&parseInt(id)>=no){
										no=id
									}
								}
							}
							if(typeof(no)=='number'){
								overrideid=itemId+"_"+no;
								name=title+no;
							}else{
								overrideid=itemId+"_"+(parseInt(no)+1);
								name=title+(parseInt(no)+1);
							}
						}else{
							overrideid=itemId+"_"+"1";
							name=title+"1";
						}
        ...
        $scope.propertyClicked = function (index) {
            if (!$scope.selectedItem.properties[index].hidden) {
            	var valus = $scope.selectedItem.properties[index].title;
            	if(valus == 'processKey' || valus == 'processName'){
            		$scope.selectedItem.properties[index].mode = "read";
            	}else{
        ...
        $scope.dropCallback = function (event, ui) {
            ...
            if ($scope.dragCanContain) {
            	...
            	else
            	{
					...
					var no=1;
					if(canvasCandidate.children.length>0){
						var shapes=canvasCandidate.children;
						var reg = /^\d+(\.\d+)?$/;
						for(var i=0;i<shapes.length;i++){
							if(shapes[i].toJSON().stencil.id!=item.id){
								continue;								
							}
							var id=shapes[i].toJSON().properties.overrideid;
							if(id){
								id=id.split("_")[1];
								if(reg.test(id)&&parseInt(id)>=no){
									no=id
						...
						if(typeof(no)=='number'){
							overrideid=item.id+"_"+no;
							name=item.name+no;
						}else{
							overrideid=item.id+"_"+(parseInt(no)+1);
							name=item.name+(parseInt(no)+1);
						}
					}else{
						overrideid=item.id+"_"+"1";
						name=item.name+"1";
					}
...
//                    alert(parentCandidate.id);
//					alert(JSON.stringify(parentCandidate.toJSON().childShapes.length));
                    $scope.editor.handleEvents({
                        type: ORYX.CONFIG.EVENT_HIGHLIGHT_HIDE,
                        highlightId: "shapeRepo.attached"
                    });
		...
		if (this.edge) {
			...
			if (edges.length > 0) {
				for (var i = 0; i < edges.length; i++) {
					if (edges[i].toJSON().stencil.id != this.edge.toJSON().stencil.id) {
						continue;
					}
					var id = edges[i].toJSON().properties.overrideid;
					if (id) {
						id=id.split("_")[1];
						if (reg.test(id) && parseInt(id) >= no) {
							no = id
				...
				if (typeof(no) == 'number') {
					overrideid = this.edge.toJSON().stencil.id+"_" + no;
					name = "连线" + no;
				} else {
					overrideid = this.edge.toJSON().stencil.id+"_"
							+ (parseInt(no) + 1);
					name = "连线" + (parseInt(no) + 1);
				}
			} else {
				overrideid = this.edge.toJSON().stencil.id+"_" + "1";
				name = "连线" + "1";
			}
			this.edge.setProperty('oryx-overrideid', overrideid);
			this.edge.setProperty('oryx-name', name);
		}
		this.facade.getCanvas().update();
		this.facade.updateSelection();
