		...
		if(sset.stencil(shapetype).type() == "node") {
			...
			if (edges.length > 0) {
				for (var i = 0; i < edges.length; i++) {
					if (edges[i].toJSON().stencil.id != newShapeObject.toJSON().stencil.id) {
						continue;
					}
				...
				if (typeof(no) == 'number') {
					overrideid = newShapeObject.toJSON().stencil.id+"_" + no;
					name = "连线" + no;
				} else {
					overrideid = newShapeObject.toJSON().stencil.id+"_"
							+ (parseInt(no) + 1);
					name = "连线"
							+ (parseInt(no) + 1);
				}
			} else {
				overrideid=newShapeObject.toJSON().stencil.id+"_"+"1";
				name="连线"+"1";
			}
			newShapeObject.setProperty('oryx-overrideid',overrideid);
			newShapeObject.setProperty('oryx-name',name);
		}
		
		if(option.name&&option.overrideid){//为节点设置默认id和name
			newShapeObject.setProperty('oryx-overrideid',option.overrideid);
			newShapeObject.setProperty('oryx-name',option.name);
		}
