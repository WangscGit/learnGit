...
jQuery.fn.pagination = function(maxentries, opts){
	opts = jQuery.extend({
		...
		isSum:false,
		isJump:false,
		jump_format_text:"Not Found！",//找不到该页码对应的页
		jump_outofrange_text:"Not Found！",//找不到该页码对应的页！
		jump_null_text:"Not Found！",//请输入页码！
		ellipse_text:"...",
	...
	return this.each(function() {
		...
		function drawLinks() {
			...
			// sum total
			if(opts.isSum){
				panel.append("<a>Count:&nbsp;"+maxentries+"&nbsp;</a>");//"<a>共&nbsp;"+maxentries+"&nbsp;条记录</a>"
			}
			// Generate "First"-Link
			if (opts.first_text
			...
			if(opts.isJump){
				// Generate Jump Input
				panel.append(jQuery("<input type='text'  style='text-align:center;font-weight:bold' id='jump-index' title='请输入正整数' size='1' />"));
